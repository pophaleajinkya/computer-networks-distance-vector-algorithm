...
def main(fname):
    ...
    routerNum = len(position.keys())  # Getting number of routers
    allTables = (
...
def router(idRouter, rTable, sharedQ, adjRouters, locks, barrier):
    ...
    global dicti
    # Initializing indexed nextHop which will be starting node
    ...
    for i in range(0, iterations):
        ...
        for adj in adjRouters:
            ...
            locks[adj].release()
        # This barrier is committed and instead locks can be used to retrieve data and
        # to maintain concurrency and data integrity
        # I have used barrier to simplify code execution and understanding
        # barrier.wait()  # Waiting for everyone to send data

        # Retrieving data and calculating new routing Table
        ...
        # return tabulate(transpose(table), headers=["Cost", "NextHop"])
